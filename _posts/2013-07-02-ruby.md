...
If you're visiting from the Internet, feel free to learn from our style. This is the internal guide we use for our own Ruby applications. We encourage you to set up one that works for your own team.

Much of this was taken from the [Github Style Guide](https://github.com/styleguide/ruby) and [Bozhidar Batsov Styleguide](https://github.com/bbatsov/ruby-style-guide) . Please add to this guide if you find any particular patterns or styles that we've adopted internally. Submit a pull request to ask for feedback (if you're an employee).
...
## <a id="coding-style">Coding Style</a>
  ...
  * Use empty lines inside method definitions to break up a method into logical paragraphs.
...
* Avoid using then for multi-line `if`/`unless` constructs.
...
* Use one expression per branch in a ternary operator. This also means that ternary operators must not be nested. Prefer `if`/`else` constructs in these cases.
  ...
  # good
  if some_condition
    if nested_condition then nested_something else nested_something_else end
  else
    something_else
  end
  ```
...
* Avoid using `unless` with `else` as some people have problems with the flow. R
...
* The `and` and `or` keywords aren't the same as `&&` and `||`. The former are
  control flow operators and the latter are boolean operators. Don't mix these.

  ``` ruby
  # good
  def some_method
    print error.message and close_thread
  end

  # good
  def some_method
    if (error.present? && error.valid?) || debug_mode?
      print environment
    end
  end
  ```
...
> Note: Some will argue that multiline chaining would look OK with the use of {...}, but they should ask themselves - is this code really readable and can't the block's contents be extracted into nifty methods?
...
> Note: While several Ruby books suggest the first style, the second is much more prominent in practice (and arguably a bit more readable).

* Do not use spaces around the `|` parameter wrapper in blocks:
...
* Using the return value of `=` (an assignment) is ok assignment with parenthesis.
...
* Avoid rescuing the Exception class. Avoid rescuing the Exception class. Avoid rescuing the Exception class. Avoid rescuing the Exception class. Avoid rescuing the Exception class. Did you not hear me? Avoid rescuing the Exception class.
...
## <a id="regular-expressions">Regular Expressions</a>
...
## <a id="percent-literals">Percent Literals</a>
...
* Use `be_true` and `be_false` instead of using `== true` and `== false`.

```ruby
# bad
Looks(2, "good", 2).should == true

# good
Looks(2, "good", 2).should be_true

# bad
Wrongs(2).should == false

# good
Wrongs(2).should be_false
```
...
based on Bozhidar Batsov's original style guide.
